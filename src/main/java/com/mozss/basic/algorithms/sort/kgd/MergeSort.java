...
public class MergeSort {
    ...
    /**
     * @title: 传入两个有序数组a和b，返回一个排好序的合并数组
     * @param: [a, b]
     * @return: int[]
     * @author: mozss
     **/
    public static int[] sort(int[] a, int[] b) {
        /**
         * @概要描述:  方法说明
         * @关键字:  三个while循环;  归并排序;  sort()方法;  递归思想;
         * @详细描述:
         *              1.第一个while: while比较数组a和数组b的元素，并将较小的赋值到数组c;
         *              2.第二个while: while循环当a数组所有元素都已经赋值到c数组之后，而b数组还有元素，那么直接把b数组剩余的元素赋值到c数组;
         *              3.第三个while: while循环则是b数组所有元素都已经赋值到c数组了，而a数组还有剩余元素,那么直接把a数组剩余的元素全部赋值到c数组;
         *
         *              归并排序的思想是把一个数组分成两半，排序每一半，然后用上面的sort()方法将数组的两半归并成为一个有序的数组。
         *              如何来为每一部分排序呢？这里我们利用递归的思想：把每一半都分为四分之一，对每个四分之一进行排序，然后把它们归并成一个
         *              有序的一半。类似的，如何给每个四分之一数组排序呢？把每个四分之一分成八分之一，对每个八分之一进行排序，以此类推，
         *              反复的分割数组，直到得到的子数组是一个数据项，那这就是这个递归算法的边界值，也就是假定一个数据项的元素是有序的。
         **/
